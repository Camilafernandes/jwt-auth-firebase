...
{
    /**
     * @var \Mockery\MockInterface|\CamilaFernandes\JWTAuth\Manager
     */
    protected $manager;

    /**
     * @var \Mockery\MockInterface|\CamilaFernandes\JWTAuth\Contracts\Providers\Auth
     */
    protected $auth;

    /**
     * @var \Mockery\MockInterface|\CamilaFernandes\JWTAuth\Http\Parser\Parser
     */
    protected $parser;
    ...
    public function it_should_return_a_token_when_passing_a_user()
    {
        ...
        $this->manager
             ...
             ->once()
             ->with(['sub' => 1, 'prv' => sha1('CamilaFernandes\JWTAuth\Test\Stubs\UserStub'), 'foo' => 'bar', 'role' => 'admin'])
    ...
    public function it_should_pass_provider_check_if_hash_matches()
    {
        ...
        $payloadFactory->shouldReceive('get')
                       ->with('prv')
                       ->andReturn(sha1('CamilaFernandes\JWTAuth\Test\Stubs\UserStub'));

        $this->manager->shouldReceive('decode')->once()->andReturn($payloadFactory);

        $this->assertTrue($this->jwtAuth->setToken('foo.bar.baz')->checkSubjectModel('CamilaFernandes\JWTAuth\Test\Stubs\UserStub'));
    }
    ...
    public function it_should_not_pass_provider_check_if_hash_not_match()
    {
        ...
        $payloadFactory->shouldReceive('get')
                       ->with('prv')
                       ->andReturn(sha1('CamilaFernandes\JWTAuth\Test\Stubs\UserStub1'));

        $this->manager->shouldReceive('decode')->once()->andReturn($payloadFactory);

        $this->assertFalse($this->jwtAuth->setToken('foo.bar.baz')->checkSubjectModel('CamilaFernandes\JWTAuth\Test\Stubs\UserStub'));
    }
